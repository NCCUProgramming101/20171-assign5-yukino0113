...
void initClocks() {
  ...
  for (int i = 0; i < clockX.length; i++) {
    ...
    while (clockX[i]==cabbageX[i]&&cabbageY[i]==clockY[i]) {
      clockX[i] = SOIL_SIZE * floor(random(SOIL_COL_COUNT));
      clockY[i] = SOIL_SIZE * ( i * 4 + floor(random(4)));
    }
  }
...
void draw() {
  ...
  case GAME_RUN: // In-Game
    ...
    for (int i = 0; i < clockX.length; i++) {
      ...
      if (isHit(playerX, playerY, SOIL_SIZE, SOIL_SIZE, clockX[i], clockY[i], SOIL_SIZE, SOIL_SIZE) == true) { // r1 bottom edge past r2 top*/
    ...
    for (int i = 0; i < soldierX.length; i++) {
      ...
      // Requirement #3: Use boolean isHit(...) to detect collision
      if (isHit(playerX, playerY, SOIL_SIZE, SOIL_SIZE, soldierX[i], soldierY[i], SOIL_SIZE, SOIL_SIZE) == true) { // r1 bottom edge past r2 top
...
String convertFramesToTimeString(int frames ) {	// Requirement #4
  ...
  return (int( nf(seconds/60, 2))  + ":" +int( nf(seconds%60, 2)));
}
